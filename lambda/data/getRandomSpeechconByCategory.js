...
async function getRandomSpeechconByCategory(locale, type) {
  ...
  const url = `https://api.airtable.com/v0/${keys.airtable_base_data}/Speechcon?api_key=${keys.airtable_api_key}&filterByFormula=AND(IsDisabled%3DFALSE(),FIND(%22${locale}%22%2C+Locale)!%3D0,FIND(%22${type}%22%2C+Type)!%3D0)`;
  const options = {
    method: "GET",
  };
